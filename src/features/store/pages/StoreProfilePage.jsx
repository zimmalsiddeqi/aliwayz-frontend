...
import {
  MapPin, Star, ShoppingBag, Users,
  UserPlus, UserMinus, Share2, Flag,
...
export default function StoreProfilePage() {
  ...
  const followMutation = useMutation({
    mutationFn: () =>
      isFollowing
        ? StoreService.unfollow(slug)
        : StoreService.follow(slug),
    onSuccess: () => {
  ...
  const handleAdminMessage = () => {
    if (products.length > 0) {
      ...
      toast(
        'This store has no products. Cannot start a conversation without a product.',
        { icon: '💬', duration: 4000 }
      );
    }
  };
  ...
  return (
    <>
      ...
      <div className="pb-24 md:pb-10">
        ...
        <div className="container-app -mt-16 relative z-10 space-y-5">
          {/* ═══ STORE HEADER ═════════════════════════════ */}
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4">
            {/* Store avatar */}
            <div
              className="rounded-2xl overflow-hidden border-4 flex-shrink-0"
              style={{
            ...
            <div className="flex-1 text-center sm:text-left min-w-0">
              ...
              {store.description && (
                <p
                  className="text-sm mt-1 line-clamp-2 max-w-lg"
                  style={{
                    color:
              ...
              {/* Meta stats */}
              <div
                className="flex flex-wrap items-center justify-center sm:justify-start gap-4 mt-2 text-xs"
                style={{
          ...
          >
            {TABS.map((tab) => (
              <button
                ...
                className={cn(
                  'flex-1 py-2 rounded-lg text-sm font-medium capitalize transition-all duration-200 whitespace-nowrap px-3 min-w-[80px]'
                )}
                style={{
                  backgroundColor:
                    activeTab === tab
            ...
            <Card className="p-5 space-y-5">
              ...
              {owner && (
                <div>
                  ...
                  <Link
                    to={`/user/${owner.username}`}
                    className="flex items-center gap-3 p-3 rounded-xl transition-colors hover:bg-[var(--glass-bg-strong)]"
                    style={{
                      border:
                      ...
                      .map((url, i) => {
                        ...
                        return (
                          <a
                            ...
                            className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl transition-colors hover:bg-[var(--glass-bg-strong)]"
                            style={{
                              color:
                                'var(--color-brand)',
                              border:
